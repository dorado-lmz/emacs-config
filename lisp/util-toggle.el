...
(defmacro spacemacs|add-toggle (name &rest props)
  "Add a toggle with NAME sysmbol.
...
`:status EXPRESSION'
    The EXPRESSION to evaluate to get current status of the toggle.
...
`:mode SYMBOL'
    if given, must be a minor mode. This override `:on', `:off' and `:status'.

All properties supported by `spacemacs//create-key-binding-form' can be used."
  (declare (indent 1))
